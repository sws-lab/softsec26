...
class User(UserMixin, db.Model):
    ...
    def can_delete_image(self, image):
        return self.is_admin and image.is_public or image.user_id == self.id
    
    def can_view_image(self, image):
        if image.is_explicit and self.is_minor():
            return False
        return image.is_public or image.user_id == self.id
...
class GuestUser(AnonymousUserMixin):
    ...
    def can_view_image(self, image):
        return image.is_public and not image.is_explicit
...
def toggle_visibility(image_id):
    ...
    if not current_user.can_delete_image(image):
...
def serve_image(filename):
    ...
    if not current_user.can_view_image(image):
        abort(403)
